...
func TestLocalReader_ReadBlock_CorrectOffset(t *testing.T) {
	...
	if n0 != BlockSize {
		t.Fatalf("ReadBlock(0) n = %d, want %d", n0, BlockSize)
	}
	if !bytes.Equal(buf0[:n0], data[:BlockSize]) {
	...
	if n1 != BlockSize {
		t.Fatalf("ReadBlock(1) n = %d, want %d", n1, BlockSize)
	}
	if !bytes.Equal(buf1[:n1], data[BlockSize:2*BlockSize]) {
	...
	if !bytes.Equal(buf2[:n2], data[2*BlockSize:]) {
...
func TestLocalReader_BlockSize_FullAndPartial(t *testing.T) {
	tests := []struct {
		...
		{"full block 0", int64(BlockSize * 3), 0, BlockSize},
		{"full block 1", int64(BlockSize * 3), 1, BlockSize},
		{"full block 2", int64(BlockSize * 3), 2, BlockSize},
		{"partial tail", int64(BlockSize + 500), 1, 500},
		{"single byte file", 1, 0, 1},
		{"exactly one block", BlockSize, 0, BlockSize},
		{"beyond end", 100, 1, 0},
		{"zero size", 0, 0, 0},
	}
...
func TestLocalReader_BlockCount_Table(t *testing.T) {
	tests := []struct {
		...
		{"zero", 0, 0},
		{"one byte", 1, 1},
		{"exactly BlockSize", BlockSize, 1},
		{"BlockSize+1", BlockSize + 1, 2},
		{"two full blocks", BlockSize * 2, 2},
		{"two blocks + 1", BlockSize*2 + 1, 3},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			r := NewLocalReader("/dev/null", tt.size)
...
func TestLocalReader_BlockCountBlockSize_Property(t *testing.T) {
	rapid.Check(t, func(t *rapid.T) {
		size := int64(rapid.IntRange(0, BlockSize*20).Draw(t, "size"))
		r := NewLocalReader("/dev/null", size)
		defer func() { _ = r.Close() }()

		// BlockCount must match BlockCount
		wantBlocks := BlockCount(size)
