...
func (c *Client) CreateFile(ctx context.Context, share *Share, parentLink *Link, name string) (*FileHandle, error) {
	mimeType := detectMIMEType(name)

	parentKR, err := parentLink.KeyRing()
	if err != nil {
		return nil, fmt.Errorf("CreateFile: parent keyring: %w", err)
	}

	addrKR, err := c.addrKRForLink(parentLink)
	if err != nil {
		return nil, fmt.Errorf("CreateFile: address keyring: %w", err)
	}

	sigAddr, err := c.signatureAddress(parentLink)
	if err != nil {
		return nil, fmt.Errorf("CreateFile: signature address: %w", err)
	}

	nodeKey, encPassphrase, passphraseSig, err := generateNodeKeys(parentKR, addrKR)
	if err != nil {
		return nil, fmt.Errorf("CreateFile: generate node keys: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("CreateFile: unlock node keyring: %w", err)
	}

	sessionKey, err := req.SetContentKeyPacketAndSignature(nodeKR)
	if err != nil {
		return nil, fmt.Errorf("CreateFile: content key packet: %w", err)
	}

	if err := req.SetName(name, addrKR, parentKR); err != nil {
		return nil, fmt.Errorf("CreateFile: encrypt name: %w", err)
	}

	hashKey, err := parentLink.ProtonLink().GetHashKeyFromParent(parentKR, addrKR)
	if err != nil {
		return nil, fmt.Errorf("CreateFile: hash key: %w", err)
	}
	if err := req.SetHash(name, hashKey); err != nil {
		return nil, fmt.Errorf("CreateFile: name hash: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("CreateFile: verification data: %w", err)
	}
	verifyCode, err := base64.StdEncoding.DecodeString(vd.VerificationCode)
	if err != nil {
		return nil, fmt.Errorf("CreateFile: decode verification code: %w", err)
	}
...
func (c *Client) OpenFile(ctx context.Context, link *Link) (*FileHandle, error) {
	if link.Type() != proton.LinkTypeFile {
		return nil, fmt.Errorf("OpenFile: %s: not a file", link.LinkID())
	}

	pLink := link.ProtonLink()
	if pLink.FileProperties == nil {
		return nil, fmt.Errorf("OpenFile: %s: no file properties", link.LinkID())
	}
...
func (c *Client) OverwriteFile(ctx context.Context, share *Share, link *Link) (*FileHandle, error) {
	if link.Type() != proton.LinkTypeFile {
		return nil, fmt.Errorf("OverwriteFile: %s: not a file", link.LinkID())
	}
	...
	nodeKR, err := link.KeyRing()
	if err != nil {
		return nil, fmt.Errorf("OverwriteFile: %s: keyring: %w", linkID, err)
	}

	pLink := link.ProtonLink()
	sessionKey, err := pLink.GetSessionKey(nodeKR)
	if err != nil {
		return nil, fmt.Errorf("OverwriteFile: %s: session key: %w", linkID, err)
	}

	addrKR, err := c.addrKRForLink(link)
	if err != nil {
		return nil, fmt.Errorf("OverwriteFile: %s: address keyring: %w", linkID, err)
	}
	...
	if err != nil {
		...
		if isDraft {
			if delErr := c.deleteStaleRevision(ctx, shareID, linkID); delErr != nil {
				return nil, fmt.Errorf("OverwriteFile: %s: delete stale draft: %w", linkID, delErr)
			}
			res, err = c.Session.Client.CreateRevision(ctx, shareID, linkID)
			if err != nil {
				return nil, fmt.Errorf("OverwriteFile: %s: create revision after draft cleanup: %w", linkID, err)
			}
		} else {
			return nil, fmt.Errorf("OverwriteFile: %s: create revision: %w", linkID, err)
		}
	}
	...
	if err != nil {
		return nil, fmt.Errorf("OverwriteFile: %s: verification data: %w", linkID, err)
	}
	verifyCode, err := base64.StdEncoding.DecodeString(vd.VerificationCode)
	if err != nil {
		return nil, fmt.Errorf("OverwriteFile: %s: decode verification code: %w", linkID, err)
	}
