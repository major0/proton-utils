...
func TestProtonReader_BlockCount(t *testing.T) {
	tests := []struct {
		...
		{"from fileSize fallback", nil, BlockSize*2 + 1, 3},
		{"single block", []int64{42}, 42, 1},
		{"empty blockSizes uses fileSize", nil, BlockSize, 1},
		{"zero file", nil, 0, 0},
	}
...
func TestProtonReader_BlockSize(t *testing.T) {
	tests := []struct {
		...
		{"fallback full block", nil, BlockSize * 3, 0, BlockSize},
		{"fallback last partial", nil, BlockSize + 100, 1, 100},
		{"fallback beyond end", nil, BlockSize, 1, 0},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			r := NewProtonReader("link1", nil, nil, tt.fileSize, tt.blockSizes, nil)
...
func TestProtonReader_BlockSize_SumEqualsTotal_Property(t *testing.T) {
	rapid.Check(t, func(t *rapid.T) {
		fileSize := int64(rapid.IntRange(1, BlockSize*10).Draw(t, "fileSize"))
