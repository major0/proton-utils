...
// table.
func (c *Client) StatLink(ctx context.Context, share *Share, parentLink *Link, linkID string) (*Link, error) {
	...
	link := NewLink(&pLink, parentLink, share, c)
...
func (c *Client) StatLinks(_ context.Context, share *Share, parentLink *Link, linkIDs []string) ([]*Link, error) {
	if len(linkIDs) == 0 {
		return nil, nil
	}
...
func (c *Client) FindLinkByName(ctx context.Context, share *Share, parentLink *Link, linkIDs []string, name string) (*Link, error) {
	if len(linkIDs) == 0 {
		return nil, nil
	}
